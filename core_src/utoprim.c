...
---------------
    ...
    the secant allows specification of complicated equation of state,
    as long as one knows 
    	pressure as a function of density and enthalpy
	pressure as a funciton of density and internal energy

    Based on Noble et al. 2006, ApJ 641, 626

    CFG 2-25-10

*/

/* if 1, keep an eye on distribution of # of inversion iterations */
#define VERBOSE_INVERSION	0

int Utoprim(double U[NPR], struct of_geom *geom, double prim[NPR])
{
	int i, iter, eflag ;
	...
	dW = -err/dedW/(1. - fmin(fmax(-0.3, f),0.3)) ;
	Wp1 = Wp ;
	err1 = err ;
		/* limit size of step */
	Wp += fmax( fmin(dW, 2.0*Wp), -0.5*Wp) ;
	err = err_eqn(Bsq, D, Ep, QdB, Qtsq, Wp, &eflag) ;
	...
	dedW2 = (errp - 2.*err + errm)/(h*h) ;
	f = 0.5*err*dedW2/(dedW*dedW) ;
	dW = -err/dedW/(1. - fmin(fmax(-0.2, f),0.2)) ;
	Wp += fmax( fmin(dW, 2.0*Wp), -0.5*Wp) ;
	...
	int *eflag) 
{
	...
	/* inversion of eq. A7 of Mignone & McKinney */

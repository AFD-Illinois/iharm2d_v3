...
void diag_log()
{
	...
	fprintf(log_file, "%15.8g %15.8g %15.8g ",
			sum_U[RHO],
			sum_U[UU],
			sum_U[U3]
			) ;
	/* output boundary fluxes of interest to file */
	/* in kerr problems this is usually mdot, edot, ldot at radial inner boundary */
	/* in a periodic box problem these might be of no interest whatsoever */
	fprintf(log_file, "%15.8g %15.8g %15.8g ",
			sum_boundary_flux[2][RHO],
			sum_boundary_flux[2][UU],
			sum_boundary_flux[2][U3]
			) ;
